...
from aspmc.parsing.meu_semantics import MEUProblogSemantics

KEYWORDS = {}  # type: ignore

class MEUProblogBuffer(Buffer):
    def __init__(
        self,
        text,
        whitespace=None,
...
class MEUProblogParser(Parser):
    ...
    def _program_(self):  # noqa
        ...
        def block0():
            with self._choice():
                ...
                with self._option():
                    self._comment_()
                self._error('expecting one of: % ?:: comment constraint decision fact normal_rule query query( rule utility utility(')
    ...
    def _weight_(self):  # noqa
        with self._choice():
            with self._option():
                self._pattern('(1(\\.0[0]*)?|0\\.[0-9]+)')
            with self._option():
                self._variable_()
            self._error('expecting one of: /(1(\\.0[0]*)?|0\\.[0-9]+)/ /[A-Z][a-zA-Z0-9]*/ variable')

    @tatsumasu()
    def _utility_(self):  # noqa
        self._token('utility(')
        with self._group():
            with self._choice():
                with self._option():
                ...
                with self._option():
                    ...
                    self._atom_()
                    self._token(')')
                self._error('expecting one of: \\+( atom')
        self._token(',')
        self._integer_()
        self._token(').')
    ...
    def _integer_(self):  # noqa
        self._pattern('((-)?(0|[1-9][0-9]*))')

    @tatsumasu()
    def _decision_(self):  # noqa
        self._token('?::')
        self._atom_()
        self._token('.')
...
def main(filename, start=None, **kwargs):
    ...
    return parser.parse(text, rule_name=start, filename=filename, semantics = MEUProblogSemantics())


if __name__ == '__main__':
    program = generic_main(main, MEUProblogParser, name='Problog')
    for rule in program:
        print(rule)
