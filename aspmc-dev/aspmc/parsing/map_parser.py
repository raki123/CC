...
from aspmc.parsing.map_semantics import MAPProblogSemantics

KEYWORDS = {}  # type: ignore


class MAPProblogBuffer(Buffer):
    def __init__(
        self,
        text,
        whitespace=None,
...
class MAPProblogParser(Parser):
    ...
    def _program_(self):  # noqa
        ...
        def block0():
            with self._choice():
                ...
                with self._option():
                    self._comment_()
                self._error('expecting one of: % comment constraint evidence evidence( fact normal_rule query query( rule')
    ...
    @tatsumasu()
    def _evidence_(self):  # noqa
        self._token('evidence(')
        self._atom_()
        self._token(').')
...
def main(filename, start=None, **kwargs):
    ...
    return parser.parse(text, rule_name=start, filename=filename, semantics = MAPProblogSemantics())


if __name__ == '__main__':
    program = generic_main(main, MAPProblogParser, name='Problog')
    for rule in program:
        print(rule)
